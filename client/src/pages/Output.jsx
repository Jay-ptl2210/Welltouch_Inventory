...
function Output() {
    ...
    const loadInitialData = async () => {
        try {
            ...
            setTransactions(transactionsRes.data.filter(t => t.type === 'delivered'));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const qtyPcs = toPcs(formData.quantity, formData.unit, selectedProduct);
            if (qtyPcs > selectedProduct.quantity) {
                throw new Error(`Insufficient stock! Available: ${selectedProduct.quantity.toFixed(1)} pcs`);
            }
            ...
            if (editingTransaction) {
                ...
                setMessage({ type: 'success', text: 'Delivery updated successfully' });
            } else {
    ...
    const calculateAvailable = () => {
        ...
        if (unit === 'packet') return (totalPcs / selectedProduct.pcsPerPacket).toFixed(1);
        ...
        <div className="container mx-auto px-4 py-8 space-y-8">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                <h1 className="text-3xl font-bold text-gray-900 mb-2">
                    {editingTransaction ? 'Edit Delivery' : 'Delivery Form'}
                </h1>
                <p className="text-gray-500 mb-8">
                    {editingTransaction ? `Editing entry from ${format(new Date(editingTransaction.date), 'dd-MM-yyyy')}` : 'Record new delivery/dispatch entries here'}
                </p>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="md:col-span-2 lg:col-span-3">
                        ...
                        <input
                            ...
                            onChange={handleFormChange}
                            className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none font-medium text-gray-700"
                            placeholder="Recipient or other info..."
                        />
                    </div>
                    ...
                    <div className="md:col-span-2 lg:col-span-1 flex items-end">
                        <button
                            type="submit"
                            disabled={loading}
                            className={`w-full ${editingTransaction ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-red-600 hover:bg-red-700'} text-white font-bold py-3 px-6 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50`}
                        >
                            {loading ? 'Processing...' : editingTransaction ? 'Update Delivery' : 'Record Delivery'}
                        </button>
                        {editingTransaction && (
                            <button
                                type="button"
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {paginatedTransactions.map((t) => (
                                <tr key={t._id} className="hover:bg-gray-50/50 transition-all">
                                    ...
                                    <td className="px-6 py-4 text-sm font-extrabold text-red-700">-{t.quantity} <span className="text-[10px] opacity-60 uppercase">{t.unit}</span></td>
